...
final class BillCycleTests: XCTestCase {
    func testRenewalDateAdvancesEachSupportedCycle() throws {
        let start = try XCTUnwrap(Calendar.billandar.date(from: DateComponents(year: 2026, month: 1, day: 15)))
        ...
        for (cycle, expectedComponents) in expectations {
            ...
            let expected = try XCTUnwrap(Calendar.billandar.date(from: expectedComponents))
