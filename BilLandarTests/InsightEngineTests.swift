...
final class InsightEngineTests: XCTestCase {
    func testDetectsHighValueProductInsights() throws {
        let now = try XCTUnwrap(Calendar.billandar.date(from: DateComponents(year: 2026, month: 8, day: 11, hour: 12)))
        ...
        trial.trialEndDate = Calendar.billandar.date(byAdding: .day, value: 2, to: now)
        let failed = makeBill(name: "Adobe", merchant: "adobe", dueIn: 4, now: now)

        let payments = [
            PaymentRecord(billID: netflix.id, billName: netflix.name, amount: 10, currencyCode: "USD", paidAt: Calendar.billandar.date(byAdding: .month, value: -2, to: now)!, status: .paid),
            PaymentRecord(billID: netflix.id, billName: netflix.name, amount: 12, currencyCode: "USD", paidAt: Calendar.billandar.date(byAdding: .month, value: -1, to: now)!, status: .paid),
    ...
    private func makeBill(
        ...
        Bill(
            ...
            cycle: .monthly,
            nextDueDate: Calendar.billandar.date(byAdding: .day, value: days, to: now)!,
