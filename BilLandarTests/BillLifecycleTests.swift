...
final class BillLifecycleTests: XCTestCase {
    func testReconcileCreatesPendingOccurrencesAdvancesDueDateAndIsIdempotent() throws {
        ...
        let firstDue = try XCTUnwrap(Calendar.billandar.date(from: DateComponents(year: 2026, month: 6, day: 11)))
        let bill = Bill(
            name: "Lifecycle",
            subtitle: "Monthly",
            amount: 12,
        ...
        XCTAssertEqual(
            Calendar.billandar.dateComponents([.year, .month, .day], from: bill.nextDueDate),
