...
struct NextPaymentWidget: Widget {
    let kind = "Billow.NextPayment"

    var body: some WidgetConfiguration {
        StaticConfiguration(kind: kind, provider: BillowWidgetProvider()) { entry in
            NextPaymentWidgetView(entry: entry)
                .environment(\.locale, BillowSharedStore.appLocale)
                .containerBackground(for: .widget) { Color(uiColor: .secondarySystemGroupedBackground) }
        }
        .configurationDisplayName("Next Payment")
        .description("See your next renewal and mark it as paid without opening Billow.")
        .supportedFamilies([.systemSmall, .systemMedium])
    }
}

struct MonthlySpendingWidget: Widget {
    let kind = "Billow.MonthlySpending"
        ...
        if let bill = entry.nextBills.first {
            if family == .systemSmall {
                ...
                HStack(spacing: 14) {
                    ...
                    VStack(alignment: .leading, spacing: 4) {
                        widgetHeader("Next payment", symbol: "calendar.badge.clock")
                        Text(bill.name).font(.headline).lineLimit(1)
                        Text(String(localized: String.LocalizationValue(bill.subtitle), locale: BillowSharedStore.appLocale))
                            .font(.caption)
            ...
            widgetEmptyState(
                message: entry.loadError
                    ?? String(localized: "No upcoming bills", locale: BillowSharedStore.appLocale)
            )
        }
    }
}
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            Text(
                entry.monthlyTotal.amount == nil
                    ? String(localized: "Shown by original currency", locale: BillowSharedStore.appLocale)
                    : String(localized: "Confirmed payments", locale: BillowSharedStore.appLocale)
            )
                .font(.caption)
                .foregroundStyle(.secondary)
            if family == .systemMedium {
                HStack(spacing: 5) {
                    ...
                    Text(String(localized: "Only payments marked paid are included", locale: BillowSharedStore.appLocale))
                }
                .font(.caption2)
...
private struct UpcomingBillsWidgetView: View {
    ...
    private var visibleBills: ArraySlice<BillowSystemBill> {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if visibleBills.isEmpty {
                ...
                widgetEmptyState(
                    message: entry.loadError
                        ?? String(localized: "Nothing due soon", locale: BillowSharedStore.appLocale)
                )
                Spacer()
            } else {
...
@ViewBuilder
private func billIcon(_ bill: BillowSystemBill, size: CGFloat) -> some View {
...
@ViewBuilder
private func paymentButton(for bill: BillowSystemBill, compact: Bool) -> some View {
    ...
    Button(intent: MarkBillPaidIntent(bill: entity)) {
        if compact {
            Image(systemName: "checkmark.circle.fill")
                ...
                .accessibilityLabel(
                    String(
                        format: String(localized: "Mark %@ paid", locale: BillowSharedStore.appLocale),
                        bill.name
                    )
                )
        } else {
...
private func dueText(for date: Date) -> String {
    if Calendar.billow.isDateInToday(date) {
        return String(localized: "Due today", locale: BillowSharedStore.appLocale)
    }
    if Calendar.billow.isDateInTomorrow(date) {
        return String(localized: "Due tomorrow", locale: BillowSharedStore.appLocale)
    }
    return String(localized: "Due", locale: BillowSharedStore.appLocale)
