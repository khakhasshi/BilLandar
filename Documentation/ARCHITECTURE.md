# BilLandar architecture

BilLandar targets iOS 17 and uses SwiftUI, SwiftData, and Swift Charts without third-party dependencies.
...
`DataStoreFactory` creates a single versioned schema containing `Bill`, `PaymentRecord`, and `PaymentMethod`. `BilLandarSchemaV1` and `BilLandarMigrationPlan` make future model changes explicit before they reach production. Production uses the user's private CloudKit database in `iCloud.JIANGJINGZHE.BilLandar`; if the CloudKit-backed container cannot be created, the app falls back to a local SwiftData store and exposes that state in Settings. Unit tests deliberately use an isolated local store.
...
Converted totals are never persisted back into a bill. If a required rate is unavailable, BilLandar keeps showing original amounts and withholds the combined total instead of silently mixing currencies.
...
The `BilLandarWidget` extension provides Next Payment, Monthly Spending, and Upcoming Bills widgets. Timeline reads use the shared SwiftData container and cached exchange rates, refresh at least every 30 minutes, and are explicitly reloaded when bills, payments, display currency, or rates change in the app.
