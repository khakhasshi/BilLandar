...
final class BillCycleTests: XCTestCase {
    func testRenewalDateAdvancesEachSupportedCycle() throws {
        let start = try XCTUnwrap(Calendar.billio.date(from: DateComponents(year: 2026, month: 1, day: 15)))
        ...
        for (cycle, expectedComponents) in expectations {
            ...
            let expected = try XCTUnwrap(Calendar.billio.date(from: expectedComponents))
