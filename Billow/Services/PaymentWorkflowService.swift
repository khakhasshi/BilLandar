...
enum PaymentWorkflowService {
    static func confirmPayment(
        ...
        if let pending = payments
            ...
            receipt = makeReceipt(
                ...
                message: String(localized: "Payment recorded", locale: BillowSharedStore.appLocale)
            )
            pending.status = .paid
            pending.paidAt = now
            pending.note = String(localized: "Confirmed manually", locale: BillowSharedStore.appLocale)
        } else {
            let payment = PaymentRecord(
                billID: bill.id,
                billName: bill.name,
                ...
                status: .paid,
                note: String(localized: "Confirmed manually", locale: BillowSharedStore.appLocale)
            )
            receipt = makeReceipt(
                payment: payment,
                bill: bill,
                insertedPayment: true,
                message: String(localized: "Payment added", locale: BillowSharedStore.appLocale)
            )
    ...
    static func update(
        ...
        let receipt = makeReceipt(
            ...
            insertedPayment: false,
            message: String(
                format: String(localized: "Payment marked %@", locale: BillowSharedStore.appLocale),
                status.title.lowercased()
            )
        )
        payment.status = status
        if status == .paid { payment.paidAt = now }
        payment.note = String(localized: "Updated manually", locale: BillowSharedStore.appLocale)
        try context.save()
        return receipt
    }
