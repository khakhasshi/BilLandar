...
final class NotificationManager {
    enum AuthorizationState: Equatable {
        ...
        var title: String {
            switch self {
            case .unknown: String(localized: "Not configured", locale: BillowSharedStore.appLocale)
            case .denied: String(localized: "Disabled in Settings", locale: BillowSharedStore.appLocale)
            case .authorized: String(localized: "Enabled", locale: BillowSharedStore.appLocale)
            case .provisional: String(localized: "Quiet delivery", locale: BillowSharedStore.appLocale)
            }
        }
    }
    ...
    func reschedule(for bills: [Bill]) async {
        ...
        let billowIDs = existing.map(\.identifier).filter { $0.hasPrefix(Keys.requestPrefix) }
        center.removePendingNotificationRequests(withIdentifiers: billowIDs)
    ...
    private func schedule(_ candidate: ReminderCandidate) async {
        ...
        let amount = bill.amount.formatted(.currency(code: bill.currencyCode).locale(BillowSharedStore.appLocale))
        let dueDate = candidate.dueDate.formatted(
            .dateTime.locale(BillowSharedStore.appLocale).month(.abbreviated).day()
        )
        content.title = String(localized: "Bill due soon", locale: BillowSharedStore.appLocale) + ": \(bill.name)"
        content.body = "\(amount) · "
            + String(localized: "Due", locale: BillowSharedStore.appLocale)
            + " \(dueDate)."
    ...
    private enum Keys {
        ...
        static let requestPrefix = "billow.bill."
    }
}
