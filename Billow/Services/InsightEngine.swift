...
enum InsightEngine {
    ...
    private static func duplicateInsights(from bills: [Bill]) -> [BillInsight] {
        ...
        .map { merchant, duplicates in
            BillInsight(
                ...
                kind: .duplicate,
                severity: .critical,
                title: String(localized: "Possible duplicate subscription", locale: BillowSharedStore.appLocale),
                message: String(
                    format: String(localized: "%@ appear to bill the same service.", locale: BillowSharedStore.appLocale),
                    duplicates.map(\.name).joined(separator: String(localized: " and ", locale: BillowSharedStore.appLocale))
                ),
    ...
    private static func priceInsights(bills: [Bill], payments: [PaymentRecord]) -> [BillInsight] {
        bills.compactMap { bill in
            ...
            return BillInsight(
                ...
                title: String(
                    format: String(localized: "%@ price increased", locale: BillowSharedStore.appLocale),
                    bill.name
                ),
                message: String(
                    format: String(localized: "Up %@ from %@ to %@.", locale: BillowSharedStore.appLocale),
                    change.formatted(.percent.precision(.fractionLength(0)).locale(BillowSharedStore.appLocale)),
                    previous.formatted(.currency(code: bill.currencyCode).locale(BillowSharedStore.appLocale)),
                    recent.amount.formatted(.currency(code: bill.currencyCode).locale(BillowSharedStore.appLocale))
                ),
                billIDs: [bill.id]
            )
        }
    ...
    private static func trialInsights(from bills: [Bill], now: Date, calendar: Calendar) -> [BillInsight] {
        bills.compactMap { bill in
            ...
            return BillInsight(
                ...
                title: String(
                    format: String(localized: "%@ trial ends soon", locale: BillowSharedStore.appLocale),
                    bill.name
                ),
                message: days == 0
                    ? String(localized: "The trial converts to a paid plan today.", locale: BillowSharedStore.appLocale)
                    : String(
                        format: String(localized: "The trial converts to %@ in %lld days.", locale: BillowSharedStore.appLocale),
                        bill.amount.formatted(.currency(code: bill.currencyCode).locale(BillowSharedStore.appLocale)),
                        days
                    ),
                billIDs: [bill.id]
            )
    ...
    private static func paymentIssueInsights(bills: [Bill], payments: [PaymentRecord]) -> [BillInsight] {
        bills.compactMap { bill in
            ...
            return BillInsight(
                ...
                kind: .paymentIssue,
                severity: .critical,
                title: String(localized: "Payment needs attention", locale: BillowSharedStore.appLocale),
                message: String(
                    format: String(localized: "The latest %@ payment failed.", locale: BillowSharedStore.appLocale),
                    bill.name
                ),
                billIDs: [bill.id]
            )
    ...
    private static func renewalClusterInsight(
        ...
        return BillInsight(
            ...
            kind: .renewalCluster,
            severity: .info,
            title: String(localized: "Busy renewal week", locale: BillowSharedStore.appLocale),
            message: String(
                format: String(localized: "%lld bills renew in the next 7 days.", locale: BillowSharedStore.appLocale),
                upcoming.count
            ),
