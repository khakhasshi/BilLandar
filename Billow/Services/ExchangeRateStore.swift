...
final class ExchangeRateStore {
    ...
    var displayCurrency: String {
        didSet {
            ...
            BillowSharedStore.defaults.set(displayCurrency, forKey: BillowSharedStore.Keys.displayCurrency)
            snapshot = nil
            historicalSnapshots = []
            errorMessage = nil
        }
    ...
    var dataStatusText: String {
        if isLoading { return String(localized: "Updating rates…", locale: BillowSharedStore.appLocale) }
        if let errorMessage { return errorMessage }
        guard let snapshot else { return String(localized: "Rates not loaded", locale: BillowSharedStore.appLocale) }
        let date = snapshot.effectiveDate.formatted(
            .dateTime.locale(BillowSharedStore.appLocale).month(.abbreviated).day().year()
        )
        return String(
            format: String(localized: "Rates from %@", locale: BillowSharedStore.appLocale),
            date
        )
    }
    ...
    func refresh() async {
        ...
        do {
            ...
            if !(error is CancellationError) {
                errorMessage = String(localized: "Using original amounts · rates unavailable", locale: BillowSharedStore.appLocale)
            }
        }
    ...
    func loadHistoricalRates(from startDate: Date, to endDate: Date, currencies: Set<String>) async {
        ...
        do {
            ...
            if !(error is CancellationError) {
                historicalErrorMessage = String(localized: "Historical rates unavailable", locale: BillowSharedStore.appLocale)
            }
        }
    ...
    private func loadCachedSnapshot() {
        guard let data = BillowSharedStore.defaults.data(forKey: cacheKey(for: displayCurrency)),
    ...
    private func saveCachedSnapshot(_ snapshot: ExchangeRateSnapshot) {
        ...
        BillowSharedStore.defaults.set(data, forKey: cacheKey(for: snapshot.baseCurrency))
    }

    private func cacheKey(for baseCurrency: String) -> String {
        "\(BillowSharedStore.Keys.exchangeRateSnapshotPrefix).\(baseCurrency)"
    }

    private static var savedDisplayCurrency: String {
        BillowSharedStore.defaults.string(forKey: BillowSharedStore.Keys.displayCurrency)
            ?? BillowSharedStore.defaults.string(forKey: "currencyCode")
            ?? "USD"
    }
}
