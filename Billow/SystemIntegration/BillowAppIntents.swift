...
struct BillEntityQuery: EntityQuery {
    ...
    func suggestedEntities() async throws -> [BillEntity] {
        let container = try BillowSystemData.makeContainer()
        let context = ModelContext(container)
        return try BillowSystemData.activeBills(in: context).prefix(10).map(BillEntity.init)
    }
}
...
struct MonthlySpendingIntent: AppIntent {
    ...
    func perform() async throws -> some IntentResult & ProvidesDialog {
        let container = try BillowSystemData.makeContainer()
        let total = try BillowSystemData.monthlySpending(in: ModelContext(container))
        return .result(dialog: "Your confirmed Billow spending this month is \(total.formatted).")
    }
}

struct AddBillIntent: AppIntent {
    static let title: LocalizedStringResource = "Add Bill"
    static let description = IntentDescription("Adds a recurring bill to Billow.")
    ...
    func perform() async throws -> some IntentResult & ProvidesDialog {
        ...
        return .result(dialog: "Added \(bill.name) to Billow.")
    }
}

struct PauseBillIntent: AppIntent {
    static let title: LocalizedStringResource = "Pause Bill"
    static let description = IntentDescription("Pauses an active recurring bill in Billow.")
    ...
    func perform() async throws -> some IntentResult & ProvidesDialog {
        let container = try BillowSystemData.makeContainer()
        let context = ModelContext(container)
        let storedBill = try BillowSystemActionService.pauseBill(id: bill.id, in: context)
        WidgetCenter.shared.reloadAllTimelines()
...
struct MarkBillPaidIntent: AppIntent {
    ...
    func perform() async throws -> some IntentResult & ProvidesDialog {
        let container = try BillowSystemData.makeContainer()
        let context = ModelContext(container)
        let storedBill = try BillowSystemActionService.markPaid(id: bill.id, in: context)
        WidgetCenter.shared.reloadAllTimelines()
...
enum BillowIntentError: LocalizedError {
    case invalidBill
    case billNotFound
    case inactiveBill
    case cancelledBill
    ...
    var errorDescription: String? {
        ...
        case .billNotFound: "That bill is no longer available in Billow."
...
#if !WIDGET_EXTENSION
struct BillowAppShortcuts: AppShortcutsProvider {
