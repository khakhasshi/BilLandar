...
struct CalendarView: View {
    ...
    private let weekdaySymbols = Calendar.billow.veryShortWeekdaySymbols
    ...
    var body: some View {
        NavigationStack {
            ...
            .billowCanvas()
            .billowNavigationTitle("Calendar")
            .task {
    ...
    private var calendarGrid: some View {
        ...
        .billowCard(padding: horizontalSizeClass == .regular ? 22 : 14)
        .simultaneousGesture(
    ...
    private func moveMonth(by value: Int) {
        guard let month = Calendar.billow.date(byAdding: .month, value: value, to: displayedMonth) else { return }
        withAnimation(reduceMotion ? nil : .snappy(duration: 0.28)) {
            displayedMonth = month
            selectedDate = Calendar.billow.dateInterval(of: .month, for: month)?.start ?? month.startOfDay
        }
