...
struct OverviewView: View {
    ...
    private var upcomingBills: [Bill] {
        let endDate = Calendar.billow.date(byAdding: .day, value: 7, to: .now) ?? .now
    ...
    var body: some View {
        NavigationStack {
            ...
            .billowNavigationTitle("Overview")
            .toolbar {
    ...
    private var summaryText: some View {
            VStack(alignment: .leading, spacing: 6) {
                ...
                if exchangeRates.isLoading && !exchangeRates.hasUsableRates {
                    BillowSkeleton(width: 154, height: 34, cornerRadius: 9)
    ...
    private var summaryChart: some View {
        Group {
            if categoryTotals.isEmpty && exchangeRates.isLoading {
                BillowSkeleton(width: 76, height: 76, cornerRadius: 38)
            } else {
    ...
    private func billGroup(date: Date, bills: [Bill]) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            .billowCard(padding: 12)
        }
    }

    private func groupTitle(for date: Date) -> String {
        if Calendar.billow.isDateInToday(date) {
            return String(localized: "Today", locale: BillowSharedStore.appLocale)
        }
        if Calendar.billow.isDateInTomorrow(date) {
            return String(localized: "Tomorrow", locale: BillowSharedStore.appLocale)
        }
        return date.formatted(
            .dateTime.locale(BillowSharedStore.appLocale).weekday(.wide).month(.abbreviated).day()
        )
    }
}
...
private struct WeekStripView: View {
    ...
    private var days: [Date] {
        (0..<7).compactMap { Calendar.billow.date(byAdding: .day, value: $0, to: .now.startOfDay) }
    }
