...
struct NotificationsView: View {
    ...
    private func notificationText(for bill: Bill) -> String {
        let days = Calendar.billow.dateComponents([.day], from: .now.startOfDay, to: bill.nextDueDate.startOfDay).day ?? 0
        let due: String
        if days <= 0 {
            due = String(localized: "today", locale: BillowSharedStore.appLocale)
        } else if days == 1 {
            due = String(localized: "tomorrow", locale: BillowSharedStore.appLocale)
        } else {
            due = String(
        ...
        return String(
            format: String(localized: "%@ is due %@", locale: BillowSharedStore.appLocale),
            amount,
            due
        )
    }
