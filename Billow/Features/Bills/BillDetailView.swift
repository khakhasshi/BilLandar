...
struct BillDetailView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 18) {
                ...
                if bill.status != .cancelled {
                    ...
                    .buttonStyle(BillowActionButtonStyle(color: AppTheme.success))
                }
                ...
                case .active:
                    HStack(spacing: 10) {
                        Button("Pause") { setStatus(.paused) }
                            .buttonStyle(BillowActionButtonStyle(color: AppTheme.accent))
                        Button("Cancel") { showingDeleteConfirmation = true }
                            .buttonStyle(BillowActionButtonStyle(color: AppTheme.danger))
                    }
                case .paused:
                    HStack(spacing: 10) {
        ...
        .alert("Cancel this bill?", isPresented: $showingDeleteConfirmation) {
            ...
            Text("Billow will stop including it in upcoming totals and reminders.")
        }
        .alert("Confirm payment?", isPresented: $showingMarkPaidConfirmation) {
            Button("Mark as Paid", action: markAsPaid)
            Button("Not Now", role: .cancel) {}
        } message: {
            Text("Billow will record a confirmed payment. You can undo the change immediately afterward.")
        }
        .task {
...
private struct InfoTile: View {
    ...
    var body: some View {
        ...
        .billowCard(padding: 14)
    }
}

private struct BillowActionButtonStyle: ButtonStyle {
    let color: Color
