...
struct AnalyticsView: View {
    ...
    private var historyStartDate: Date {
        Calendar.billow.date(byAdding: .month, value: -5, to: Date.now.startOfDay) ?? Date.now
    }
    ...
    private var monthlySpending: [(month: Date, amount: Double)] {
        let converted = historicalPayments.compactMap { payment -> (Date, Double)? in
            guard let month = Calendar.billow.dateInterval(of: .month, for: payment.paidAt)?.start,
    ...
    var body: some View {
        NavigationStack {
            ...
            .billowNavigationTitle("Analytics")
            .toolbar {
            ...
            .task(id: historyLoadKey) {
                ...
                await exchangeRates.loadHistoricalRates(
                    from: Calendar.billow.date(byAdding: .day, value: -7, to: historyStartDate) ?? historyStartDate,
                    to: .now,
    ...
    private var spendingHistoryCard: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            if exchangeRates.isLoadingHistory {
                VStack(spacing: 12) {
                    BillowSkeleton(height: 138, cornerRadius: 14)
                    HStack {
                        BillowSkeleton(width: 54, height: 10)
                        Spacer()
                        BillowSkeleton(width: 54, height: 10)
                    }
                }
