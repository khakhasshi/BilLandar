...
struct BillioSystemBill: Identifiable, Hashable, Sendable {
    let id: UUID
    let name: String
    let subtitle: String
    let amount: Double
...
struct BillioSystemTotal: Sendable {
    let amount: Double?
    let currencyCode: String
...
enum BillioSystemData {
    ...
    static func nextBills(limit: Int, now: Date = .now, in context: ModelContext) throws -> [BillioSystemBill] {
        Array(try activeBills(in: context).filter { $0.nextDueDate >= now.startOfDay }.prefix(limit))
            .map(BillioSystemBill.init)
    }

    static func monthlySpending(now: Date = .now, in context: ModelContext) throws -> BillioSystemTotal {
        let calendar = Calendar.billio
        guard let interval = calendar.dateInterval(of: .month, for: now) else {
            return BillioSystemTotal(amount: 0, currencyCode: displayCurrency, originalCurrencyBreakdown: [:])
        }
    ...
    static func total(for values: [(Double, String)]) -> BillioSystemTotal {
        ...
        guard let snapshot = cachedRates, snapshot.baseCurrency == base else {
            if breakdown.count == 1, let only = breakdown.first {
                return BillioSystemTotal(amount: only.value, currencyCode: only.key, originalCurrencyBreakdown: breakdown)
            }
            return BillioSystemTotal(amount: nil, currencyCode: base, originalCurrencyBreakdown: breakdown)
        }
        ...
        return BillioSystemTotal(amount: convertedTotal, currencyCode: base, originalCurrencyBreakdown: breakdown)
    }

    static var displayCurrency: String {
        BillioSharedStore.defaults.string(forKey: BillioSharedStore.Keys.displayCurrency) ?? "USD"
    }

    private static var cachedRates: ExchangeRateSnapshot? {
        let key = "\(BillioSharedStore.Keys.exchangeRateSnapshotPrefix).\(displayCurrency)"
        guard let data = BillioSharedStore.defaults.data(forKey: key) else { return nil }
...
@MainActor
enum BillioSystemActionService {
    @discardableResult
    static func addBill(
        name: String,
        amount: Double,
        ...
        guard !trimmed.isEmpty, amount > 0 else { throw BillioIntentError.invalidBill }
        let bill = Bill(
            name: trimmed,
    ...
    static func pauseBill(id: UUID, in context: ModelContext) throws -> Bill {
        guard let bill = try BillioSystemData.bill(id: id, in: context) else {
            throw BillioIntentError.billNotFound
        }
        guard bill.status != .cancelled else { throw BillioIntentError.cancelledBill }
        bill.status = .paused
        try context.save()
        return bill
    }
    ...
    static func markPaid(id: UUID, in context: ModelContext, now: Date = .now) throws -> Bill {
        ...
        guard bill.status == .active else { throw BillioIntentError.inactiveBill }
        let payments = try BillioSystemData.payments(for: bill.id, in: context)
