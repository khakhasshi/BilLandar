...
struct BillEntityQuery: EntityQuery {
    ...
    func suggestedEntities() async throws -> [BillEntity] {
        let container = try BillioSystemData.makeContainer()
        let context = ModelContext(container)
        return try BillioSystemData.activeBills(in: context).prefix(10).map(BillEntity.init)
    }
}
...
struct MonthlySpendingIntent: AppIntent {
    ...
    func perform() async throws -> some IntentResult & ProvidesDialog {
        let container = try BillioSystemData.makeContainer()
        let total = try BillioSystemData.monthlySpending(in: ModelContext(container))
        return .result(dialog: "Your confirmed Billio spending this month is \(total.formatted).")
    }
}

struct AddBillIntent: AppIntent {
    static let title: LocalizedStringResource = "Add Bill"
    static let description = IntentDescription("Adds a recurring bill to Billio.")
    ...
    func perform() async throws -> some IntentResult & ProvidesDialog {
        ...
        return .result(dialog: "Added \(bill.name) to Billio.")
    }
}

struct PauseBillIntent: AppIntent {
    static let title: LocalizedStringResource = "Pause Bill"
    static let description = IntentDescription("Pauses an active recurring bill in Billio.")
    ...
    func perform() async throws -> some IntentResult & ProvidesDialog {
        let container = try BillioSystemData.makeContainer()
        let context = ModelContext(container)
        let storedBill = try BillioSystemActionService.pauseBill(id: bill.id, in: context)
        WidgetCenter.shared.reloadAllTimelines()
...
struct MarkBillPaidIntent: AppIntent {
    ...
    func perform() async throws -> some IntentResult & ProvidesDialog {
        let container = try BillioSystemData.makeContainer()
        let context = ModelContext(container)
        let storedBill = try BillioSystemActionService.markPaid(id: bill.id, in: context)
        WidgetCenter.shared.reloadAllTimelines()
...
enum BillioIntentError: LocalizedError {
    case invalidBill
    case billNotFound
    case inactiveBill
    case cancelledBill
    ...
    var errorDescription: String? {
        ...
        case .billNotFound: "That bill is no longer available in Billio."
...
#if !WIDGET_EXTENSION
struct BillioAppShortcuts: AppShortcutsProvider {
