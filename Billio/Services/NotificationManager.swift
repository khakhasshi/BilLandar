...
final class NotificationManager {
    ...
    func reschedule(for bills: [Bill]) async {
        ...
        let billioIDs = existing.map(\.identifier).filter { $0.hasPrefix(Keys.requestPrefix) }
        center.removePendingNotificationRequests(withIdentifiers: billioIDs)
    ...
    private enum Keys {
        ...
        static let requestPrefix = "billio.bill."
    }
}
