...
enum AppThemeMode: String, CaseIterable, Identifiable {
    ...
    var title: String {
        switch self {
        case .system: String(localized: "System", locale: BillioSharedStore.appLocale)
        case .light: String(localized: "Light", locale: BillioSharedStore.appLocale)
        case .dark: String(localized: "Dark", locale: BillioSharedStore.appLocale)
        }
    }
...
/// The app-level language choice. `system` keeps Billio aligned with the device
...
final class AppLanguageStore {
    var language: AppLanguage {
        didSet {
            ...
            BillioSharedStore.defaults.set(language.rawValue, forKey: BillioSharedStore.Keys.languageIdentifier)
    ...
    init() {
        BillioSharedStore.migrateLegacyDefaultsIfNeeded()
        let saved = BillioSharedStore.defaults.string(forKey: BillioSharedStore.Keys.languageIdentifier)
