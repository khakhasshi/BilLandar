...
struct BilLandarSystemBill: Identifiable, Hashable, Sendable {
    let id: UUID
    let name: String
    let subtitle: String
    let amount: Double
...
struct BilLandarSystemTotal: Sendable {
    let amount: Double?
    let currencyCode: String
    ...
    var formatted: String {
        ...
        if originalCurrencyBreakdown.isEmpty {
            return String(localized: "No payments", locale: BilLandarSharedStore.appLocale)
        }
...
enum BilLandarSystemData {
    ...
    static func nextBills(limit: Int, now: Date = .now, in context: ModelContext) throws -> [BilLandarSystemBill] {
        Array(try activeBills(in: context).filter { $0.nextDueDate >= now.startOfDay }.prefix(limit))
            .map(BilLandarSystemBill.init)
    }

    static func monthlySpending(now: Date = .now, in context: ModelContext) throws -> BilLandarSystemTotal {
        let calendar = Calendar.billandar
        guard let interval = calendar.dateInterval(of: .month, for: now) else {
            return BilLandarSystemTotal(amount: 0, currencyCode: displayCurrency, originalCurrencyBreakdown: [:])
        }
    ...
    static func total(for values: [(Double, String)]) -> BilLandarSystemTotal {
        ...
        guard let snapshot = cachedRates, snapshot.baseCurrency == base else {
            if breakdown.count == 1, let only = breakdown.first {
                return BilLandarSystemTotal(amount: only.value, currencyCode: only.key, originalCurrencyBreakdown: breakdown)
            }
            return BilLandarSystemTotal(amount: nil, currencyCode: base, originalCurrencyBreakdown: breakdown)
        }
        ...
        return BilLandarSystemTotal(amount: convertedTotal, currencyCode: base, originalCurrencyBreakdown: breakdown)
    }

    static var displayCurrency: String {
        BilLandarSharedStore.defaults.string(forKey: BilLandarSharedStore.Keys.displayCurrency) ?? "USD"
    }

    private static var cachedRates: ExchangeRateSnapshot? {
        let key = "\(BilLandarSharedStore.Keys.exchangeRateSnapshotPrefix).\(displayCurrency)"
        guard let data = BilLandarSharedStore.defaults.data(forKey: key) else { return nil }
...
@MainActor
enum BilLandarSystemActionService {
    @discardableResult
    static func addBill(
        name: String,
        amount: Double,
        ...
        guard !trimmed.isEmpty, amount > 0 else { throw BilLandarIntentError.invalidBill }
        let bill = Bill(
            name: trimmed,
    ...
    static func pauseBill(id: UUID, in context: ModelContext) throws -> Bill {
        guard let bill = try BilLandarSystemData.bill(id: id, in: context) else {
            throw BilLandarIntentError.billNotFound
        }
        guard bill.status != .cancelled else { throw BilLandarIntentError.cancelledBill }
        bill.status = .paused
        try context.save()
        return bill
    }
    ...
    static func markPaid(id: UUID, in context: ModelContext, now: Date = .now) throws -> Bill {
        ...
        guard bill.status == .active else { throw BilLandarIntentError.inactiveBill }
        let payments = try BilLandarSystemData.payments(for: bill.id, in: context)
