...
final class ExchangeRateStore {
    ...
    var displayCurrency: String {
        didSet {
            ...
            BilLandarSharedStore.defaults.set(displayCurrency, forKey: BilLandarSharedStore.Keys.displayCurrency)
            snapshot = nil
            historicalSnapshots = []
            errorMessage = nil
        }
    ...
    var dataStatusText: String {
        if isLoading { return String(localized: "Updating rates…", locale: BilLandarSharedStore.appLocale) }
        if let errorMessage { return errorMessage }
        guard let snapshot else { return String(localized: "Rates not loaded", locale: BilLandarSharedStore.appLocale) }
        let date = snapshot.effectiveDate.formatted(
            .dateTime.locale(BilLandarSharedStore.appLocale).month(.abbreviated).day().year()
        )
        return String(
            format: String(localized: "Rates from %@", locale: BilLandarSharedStore.appLocale),
            date
        )
    }
    ...
    func refresh() async {
        ...
        do {
            ...
            if !(error is CancellationError) {
                errorMessage = String(localized: "Using original amounts · rates unavailable", locale: BilLandarSharedStore.appLocale)
            }
        }
    ...
    func loadHistoricalRates(from startDate: Date, to endDate: Date, currencies: Set<String>) async {
        ...
        do {
            ...
            if !(error is CancellationError) {
                historicalErrorMessage = String(localized: "Historical rates unavailable", locale: BilLandarSharedStore.appLocale)
            }
        }
    ...
    private func loadCachedSnapshot() {
        guard let data = BilLandarSharedStore.defaults.data(forKey: cacheKey(for: displayCurrency)),
    ...
    private func saveCachedSnapshot(_ snapshot: ExchangeRateSnapshot) {
        ...
        BilLandarSharedStore.defaults.set(data, forKey: cacheKey(for: snapshot.baseCurrency))
    }

    private func cacheKey(for baseCurrency: String) -> String {
        "\(BilLandarSharedStore.Keys.exchangeRateSnapshotPrefix).\(baseCurrency)"
    }

    private static var savedDisplayCurrency: String {
        BilLandarSharedStore.defaults.string(forKey: BilLandarSharedStore.Keys.displayCurrency)
            ?? BilLandarSharedStore.defaults.string(forKey: "currencyCode")
            ?? "USD"
    }
}
