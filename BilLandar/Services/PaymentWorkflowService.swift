...
enum PaymentWorkflowService {
    static func confirmPayment(
        ...
        if let pending = payments
            ...
            receipt = makeReceipt(
                ...
                message: String(localized: "Payment recorded", locale: BilLandarSharedStore.appLocale)
            )
            pending.status = .paid
            pending.paidAt = now
            pending.note = String(localized: "Confirmed manually", locale: BilLandarSharedStore.appLocale)
        } else {
            let payment = PaymentRecord(
                billID: bill.id,
                billName: bill.name,
                ...
                status: .paid,
                note: String(localized: "Confirmed manually", locale: BilLandarSharedStore.appLocale)
            )
            receipt = makeReceipt(
                payment: payment,
                bill: bill,
                insertedPayment: true,
                message: String(localized: "Payment added", locale: BilLandarSharedStore.appLocale)
            )
    ...
    static func update(
        ...
        let receipt = makeReceipt(
            ...
            insertedPayment: false,
            message: String(
                format: String(localized: "Payment marked %@", locale: BilLandarSharedStore.appLocale),
                status.title.lowercased()
            )
        )
        payment.status = status
        if status == .paid { payment.paidAt = now }
        payment.note = String(localized: "Updated manually", locale: BilLandarSharedStore.appLocale)
        try context.save()
        return receipt
    }
