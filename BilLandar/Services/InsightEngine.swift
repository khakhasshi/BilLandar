...
enum InsightEngine {
    ...
    private static func duplicateInsights(from bills: [Bill]) -> [BillInsight] {
        ...
        .map { merchant, duplicates in
            BillInsight(
                ...
                kind: .duplicate,
                severity: .critical,
                title: String(localized: "Possible duplicate subscription", locale: BilLandarSharedStore.appLocale),
                message: String(
                    format: String(localized: "%@ appear to bill the same service.", locale: BilLandarSharedStore.appLocale),
                    duplicates.map(\.name).joined(separator: String(localized: " and ", locale: BilLandarSharedStore.appLocale))
                ),
    ...
    private static func priceInsights(bills: [Bill], payments: [PaymentRecord]) -> [BillInsight] {
        bills.compactMap { bill in
            ...
            return BillInsight(
                ...
                title: String(
                    format: String(localized: "%@ price increased", locale: BilLandarSharedStore.appLocale),
                    bill.name
                ),
                message: String(
                    format: String(localized: "Up %@ from %@ to %@.", locale: BilLandarSharedStore.appLocale),
                    change.formatted(.percent.precision(.fractionLength(0)).locale(BilLandarSharedStore.appLocale)),
                    previous.formatted(.currency(code: bill.currencyCode).locale(BilLandarSharedStore.appLocale)),
                    recent.amount.formatted(.currency(code: bill.currencyCode).locale(BilLandarSharedStore.appLocale))
                ),
                billIDs: [bill.id]
            )
        }
    ...
    private static func trialInsights(from bills: [Bill], now: Date, calendar: Calendar) -> [BillInsight] {
        bills.compactMap { bill in
            ...
            return BillInsight(
                ...
                title: String(
                    format: String(localized: "%@ trial ends soon", locale: BilLandarSharedStore.appLocale),
                    bill.name
                ),
                message: days == 0
                    ? String(localized: "The trial converts to a paid plan today.", locale: BilLandarSharedStore.appLocale)
                    : String(
                        format: String(localized: "The trial converts to %@ in %lld days.", locale: BilLandarSharedStore.appLocale),
                        bill.amount.formatted(.currency(code: bill.currencyCode).locale(BilLandarSharedStore.appLocale)),
                        days
                    ),
                billIDs: [bill.id]
            )
    ...
    private static func paymentIssueInsights(bills: [Bill], payments: [PaymentRecord]) -> [BillInsight] {
        bills.compactMap { bill in
            ...
            return BillInsight(
                ...
                kind: .paymentIssue,
                severity: .critical,
                title: String(localized: "Payment needs attention", locale: BilLandarSharedStore.appLocale),
                message: String(
                    format: String(localized: "The latest %@ payment failed.", locale: BilLandarSharedStore.appLocale),
                    bill.name
                ),
                billIDs: [bill.id]
            )
    ...
    private static func renewalClusterInsight(
        ...
        return BillInsight(
            ...
            kind: .renewalCluster,
            severity: .info,
            title: String(localized: "Busy renewal week", locale: BilLandarSharedStore.appLocale),
            message: String(
                format: String(localized: "%lld bills renew in the next 7 days.", locale: BilLandarSharedStore.appLocale),
                upcoming.count
            ),
