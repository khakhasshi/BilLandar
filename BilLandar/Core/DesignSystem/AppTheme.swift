...
extension View {
    func billandarCard(padding: CGFloat = 16) -> some View {
        modifier(BilLandarCardModifier(padding: padding))
    }

    func billandarCanvas() -> some View {
        self.background(AppTheme.canvas.ignoresSafeArea())
    }

    func billandarTouchTarget() -> some View {
        frame(minWidth: AppTheme.minimumTouchSize, minHeight: AppTheme.minimumTouchSize)
            .contentShape(Rectangle())
    }

    func billandarTabBarClearance() -> some View {
        safeAreaPadding(.bottom, AppTheme.tabBarClearance)
    }

    func billandarNavigationTitle(_ title: String) -> some View {
...
private struct BilLandarCardModifier: ViewModifier {
