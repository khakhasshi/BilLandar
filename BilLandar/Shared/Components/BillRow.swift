...
struct BillRow: View {
    ...
    private var dueBadge: some View {
        let days = Calendar.billandar.dateComponents(
            [.day],
        ...
        let title: String
        if days <= 0 {
            title = String(localized: "Due today", locale: BilLandarSharedStore.appLocale)
        } else if days == 1 {
            title = String(localized: "Due tomorrow", locale: BilLandarSharedStore.appLocale)
        } else {
            title = String(
