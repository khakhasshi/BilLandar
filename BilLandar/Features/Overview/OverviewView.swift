...
struct OverviewView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .billandarNavigationTitle("Overview")
            .toolbar {
    ...
    private var summaryText: some View {
            VStack(alignment: .leading, spacing: 6) {
                ...
                if exchangeRates.isLoading && !exchangeRates.hasUsableRates {
                    BilLandarSkeleton(width: 154, height: 34, cornerRadius: 9)
    ...
    private var summaryChart: some View {
        Group {
            if categoryTotals.isEmpty && exchangeRates.isLoading {
                BilLandarSkeleton(width: 76, height: 76, cornerRadius: 38)
            } else {
    ...
    private func groupTitle(for date: Date) -> String {
        if Calendar.billandar.isDateInToday(date) {
            return String(localized: "Today", locale: BilLandarSharedStore.appLocale)
        }
        if Calendar.billandar.isDateInTomorrow(date) {
            return String(localized: "Tomorrow", locale: BilLandarSharedStore.appLocale)
        }
        return date.formatted(
            .dateTime.locale(BilLandarSharedStore.appLocale).weekday(.wide).month(.abbreviated).day()
        )
    }
}
...
private struct WeekStripView: View {
    ...
    private var days: [Date] {
        (0..<7).compactMap { Calendar.billandar.date(byAdding: .day, value: $0, to: .now.startOfDay) }
    }
