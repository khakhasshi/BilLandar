...
struct AddBillView: View {
    ...
    @State private var nextDueDate = Date.billandarDate(daysFromToday: 1)
    @State private var notes = ""
    @State private var hasTrial = false
    @State private var trialEndDate = Date.billandarDate(daysFromToday: 7)
    ...
    private var hasUnsavedChanges: Bool {
        !name.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
            ...
            || cycle != .monthly
            || !nextDueDate.isSameDay(as: .billandarDate(daysFromToday: 1))
            || !notes.isEmpty
            || hasTrial
