...
struct NextPaymentWidget: Widget {
    let kind = "BilLandar.NextPayment"

    var body: some WidgetConfiguration {
        StaticConfiguration(kind: kind, provider: BilLandarWidgetProvider()) { entry in
            NextPaymentWidgetView(entry: entry)
                .environment(\.locale, BilLandarSharedStore.appLocale)
                .containerBackground(for: .widget) { Color(uiColor: .secondarySystemGroupedBackground) }
        }
        .configurationDisplayName("Next Payment")
        .description("See your next renewal and mark it as paid without opening BilLandar.")
        .supportedFamilies([.systemSmall, .systemMedium])
    }
}

struct MonthlySpendingWidget: Widget {
    let kind = "BilLandar.MonthlySpending"
...
struct UpcomingBillsWidget: Widget {
    let kind = "BilLandar.UpcomingBills"
...
private struct NextPaymentWidgetView: View {
    ...
    let entry: BilLandarWidgetEntry
        ...
        if let bill = entry.nextBills.first {
            if family == .systemSmall {
                ...
                HStack(spacing: 14) {
                    ...
                    VStack(alignment: .leading, spacing: 4) {
                        widgetHeader("Next payment", symbol: "calendar.badge.clock")
                        Text(bill.name).font(.headline).lineLimit(1)
                        Text(String(localized: String.LocalizationValue(bill.subtitle), locale: BilLandarSharedStore.appLocale))
                            .font(.caption)
            ...
            widgetEmptyState(
                message: entry.loadError
                    ?? String(localized: "No upcoming bills", locale: BilLandarSharedStore.appLocale)
            )
        }
    }
}
        ...
        VStack(alignment: .leading, spacing: 10) {
            ...
            Text(
                entry.monthlyTotal.amount == nil
                    ? String(localized: "Shown by original currency", locale: BilLandarSharedStore.appLocale)
                    : String(localized: "Confirmed payments", locale: BilLandarSharedStore.appLocale)
            )
                .font(.caption)
                .foregroundStyle(.secondary)
            if family == .systemMedium {
                HStack(spacing: 5) {
                    ...
                    Text(String(localized: "Only payments marked paid are included", locale: BilLandarSharedStore.appLocale))
                }
                .font(.caption2)
...
private struct UpcomingBillsWidgetView: View {
    ...
    private var visibleBills: ArraySlice<BilLandarSystemBill> {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if visibleBills.isEmpty {
                ...
                widgetEmptyState(
                    message: entry.loadError
                        ?? String(localized: "Nothing due soon", locale: BilLandarSharedStore.appLocale)
                )
                Spacer()
            } else {
...
@ViewBuilder
private func billIcon(_ bill: BilLandarSystemBill, size: CGFloat) -> some View {
...
@ViewBuilder
private func paymentButton(for bill: BilLandarSystemBill, compact: Bool) -> some View {
    ...
    Button(intent: MarkBillPaidIntent(bill: entity)) {
        if compact {
            Image(systemName: "checkmark.circle.fill")
                ...
                .accessibilityLabel(
                    String(
                        format: String(localized: "Mark %@ paid", locale: BilLandarSharedStore.appLocale),
                        bill.name
                    )
                )
        } else {
...
private func dueText(for date: Date) -> String {
    if Calendar.billandar.isDateInToday(date) {
        return String(localized: "Due today", locale: BilLandarSharedStore.appLocale)
    }
    if Calendar.billandar.isDateInTomorrow(date) {
        return String(localized: "Due tomorrow", locale: BilLandarSharedStore.appLocale)
    }
    return String(localized: "Due", locale: BilLandarSharedStore.appLocale)
